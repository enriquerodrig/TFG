...
def login(username, password):
    ...
    if response.status_code in (302, 303):
        ...
        if 'add-phone' in redirect:
            ...
            print("[!] Some unknown redirection occurred. If this persists, please open an issue to LinkedIn2Username on github. "
...
def set_outer_loops(geoblast=[], keywords=[]):
    """
    ...
    if geoblast:
        ...
        outer_loops = range(0, len(keywords))
    else:
...
def do_loops(session, company_id, outer_loops, geoblast=[], keywords=[], depth=0, jitter=0, needed_employees=None):
    """
    ...
    try:
        for current_loop in outer_loops:
            if geoblast:
                ...
                current_keyword = keywords[current_loop]
                current_region = ''
            ...
            for page in range(0, depth):
                ...
                if needed_employees and len(employee_list) >= needed_employees:
                    print(f"\n[*] We have enough names! Moving on...")
                    break
        ...
        print("\n\n[!] Caught Ctrl-C. Breaking loops and writing output collected so far...")
...
def write_files(company, domain, employees, out_dir):
    ...
    with open(f'{out_dir}/{company}-metadata.txt', 'w', encoding='utf-8') as outfile:
        ...
        for employee in employees:
            outfile.write(employee['full_name'] + ',' + employee["position"] + '\n')
...
def main():
    ...
    employees = do_loops(session, company_id, outer_loops, args.geoblast, args.keywords, args.depth)
